...
from .utils import hash_password, verify_password, calculate_cart_total, calculate_order_total
...
def login(request):
    ...
    if request.POST:
        ...
        try:
            uid=user.objects.get(email=email)
            if verify_password(password, uid.password):
        ...
        try:
            ...
            if str(uid.otp)==otp:
                if new_password==con_password:
                    uid.password=hash_password(new_password)
                    uid.save()
...
def shop(request):
    ...
    pcount=product1.objects.count()
    pro1=wishlist.objects.filter(user_id=uid)
    wid=wishlist.objects.filter(user_id=uid)
    list1=[]
    for i in wid:
        list1.append(i.product_id.id)
    print(list1)    
    ...
    if scid:
        ...
        pcount=product1.objects.filter(subcategory=scid).count()
        print(pcount) 
        
    elif bcid:
        pid=product1.objects.filter(brand=bcid)
        pcount=product1.objects.filter(brand=bcid).count()
        print(pcount)           
    elif pcid:
...
def profile(request):
    ...
    if request.POST:
        ...
        try:
            if verify_password(old_password, uid.password):
                uid.password=hash_password(new_password)
                uid.save()
